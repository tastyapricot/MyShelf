...
fun EditBookScreen(
    ...
    LaunchedEffect(Unit) {
        navBackStackEntry.savedStateHandle.get<Genre>(Screen.NewBook.KEY_GENRE)
            ?.let { genre -> viewModel.onIntent(EditBookContract.Intent.OnGenreSelected(genre)) }

        viewModel.labels.onEach { label ->
            when (label) {
                EditBookContract.Label.Back -> navController.popBackStack()
                EditBookContract.Label.Save -> navController.popBackStack()
                EditBookContract.Label.SelectGenre -> navController.navigate(Screen.SelectGenre)
            }
        }
            .launchIn(this)
    }
...
private fun EditBookScreen(
    state: EditBookContract.State,
    onIntent: (EditBookContract.Intent) -> Unit,
) {
    Scaffold(
        topBar = {
            TopAppBar(
                title = {
                    Text(
                        text = stringResource(id = R.string.edit_book_title),
                        maxLines = 1,
                        overflow = TextOverflow.Ellipsis
                    )
                },
                navigationIcon = {
                    IconButton(onClick = { onIntent(EditBookContract.Intent.Back) }) {
                        Icon(
                ...
                actions = {
                    IconButton(onClick = { onIntent(EditBookContract.Intent.Delete) }) {
                        Icon(
                            imageVector = Icons.Filled.Delete,
                            contentDescription = stringResource(R.string.delete),
                        )
                    }
                    IconButton(onClick = { onIntent(EditBookContract.Intent.Save) }) {
                        Icon(
        ...
        Box(
            ...
            Column(
                ...
                OutlinedTextField(
                    ...
                    onValueChange = { onIntent(EditBookContract.Intent.OnTitleChange(it)) },
                ...
                OutlinedTextField(
                    ...
                    onValueChange = { onIntent(EditBookContract.Intent.OnAuthorChange(it)) },
                ...
                OutlinedTextField(
                    ...
                    onValueChange = { onIntent(EditBookContract.Intent.OnDescriptionChange(it)) },
                            ...
                            LaunchedEffect(interactionSource) {
                                interactionSource.interactions.collect {
                                    if (it is PressInteraction.Release) {
                                        onIntent(EditBookContract.Intent.SelectGenre)
                                    }
                                }
                            }
                        },
